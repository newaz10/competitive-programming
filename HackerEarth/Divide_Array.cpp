...
int main() {
    ...
    // Initialize a variable to accumulate the total divisor effect
    ll totalDivisor = 1;

    // For each operation, accumulate the divisor effect
    for (int i = 0; i < Q; ++i) {
        ll D;
        cin >> D;

        // Multiply cumulative divisor and take modulus to avoid overflow
        totalDivisor = (totalDivisor * D) % 1000000007;
    }

    // Now apply the total divisor effect to each element of the array
    for (int i = 0; i < N; ++i) {
        A[i] /= totalDivisor;
    }
